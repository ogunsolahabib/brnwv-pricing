...
export default function Button({ children, variant = "primary", className, ...props }: { children: React.ReactNode, variant?: "primary" | "secondary" } & React.ButtonHTMLAttributes<HTMLButtonElement>) {
    return <button className={`${getVariantClasses(variant)} ${className}`} {...props}>{children}</button>
}